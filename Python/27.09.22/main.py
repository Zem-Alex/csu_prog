...
class MyGame(arcade.Window):
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.UP:
            ...
            if PLATFORM_SPEED == 200:
                PLATFORM_SPEED = 1
            else:
                PLATFORM_SPEED = 200
        if key != arcade.key.UP and key != arcade.key.LEFT and key != arcade.key.RIGHT:
            self.moving_wall_list[0].change_x = PLATFORM_SPEED
            self.moving_wall_list[1].change_y = PLATFORM_SPEED
