    ...
    switch (iMsg){
        ...
        case WM_COMMAND:
            ...
            switch (LOWORD (wParam)) {
                ...
                case IDM_LIGHTER:
                    if (glCurrentBitmap) {
                        newBitmap = changeBitmapL(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                case IDM_DARKER:
                    if (glCurrentBitmap) {
                        newBitmap = changeBitmapD(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                case IDM_BLACKWHITE:
                    if (glCurrentBitmap) {
                        newBitmap = changeBitmapBW(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                case IDM_FLIP:
                    if (glCurrentBitmap) {
                        newBitmap = flipBitmap(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                case IDM_GRID:
                    if (glCurrentBitmap) {
                        newBitmap = Grid(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                default:
